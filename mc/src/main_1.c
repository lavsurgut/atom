...
static float magnHeadingCalc(uint8_t *msgPtr);

static float magnReadValues(void);
...
//Magnetometer CRA register configuration
uint8_t cmdMagnWriteCRA[2] = {TW_CMD_MAGN_WRITE, 0x00};
uint8_t msgMagnWriteCRA[1] = {0x10};
//Magnetometer CRB register configuration
uint8_t cmdMagnWriteCRB[2] = {TW_CMD_MAGN_WRITE, 0x01};
uint8_t msgMagnWriteCRB[1] = {0x40};
...
int main(void) {
    ...
    I2CDo(msgMagnWriteCRA, 1, cmdMagnWriteCRA, 2);

    I2CDo(msgMagnWriteCRB, 1, cmdMagnWriteCRB, 2);
    ...
    while (1) {
        ...
        magnHeadingDegrees = magnReadValues();
        //mpu6050ReadAcc();

        msgUART[0] = (uint8_t) (((int16_t)magnHeadingDegrees) >> 8);
        msgUART[1] = (uint8_t) (((int16_t)magnHeadingDegrees) & 0x00FF);
...
float magnHeadingCalc(uint8_t *msgPtr) {
    ...
    float gainFact = 1.22;
    //float xGainError = 1.02;
    float xGainError = 1.03;
    float yGainError = 1.06;
    //float xOffset = -39.66;
    //float yOffset = 193.45;
    float xOffset = -89.76;
    float yOffset = -29.76;

    x = (int16_t) (msgPtr[0] << 8) | msgPtr[1];
    y = (int16_t) (msgPtr[4] << 8) | msgPtr[5];

    xScaled = x * gainFact * xGainError + xOffset;
    yScaled = y * gainFact * yGainError + yOffset;


    heading = atan2(yScaled, xScaled);

    headingDegrees = (heading * (180 / PI)) + 180;

    return headingDegrees;
}

float magnReadValues(void) {
    ...
    magnHeadingDegrees = magnHeadingCalc(msgMagnRead);
